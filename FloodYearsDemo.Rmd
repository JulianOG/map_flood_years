---
title: "Mapping storm tide flood years for Northern Australia"
...
selectpts = c(which(voi_id > 4000), which(voi_id == 3064))

xs = xs[selectpts]

voi_id = voi_id[selectpts]
...
map <- reduce(seq_along(rl), function(m, i) {
  m %>% addRasterImage(rl[[i]], colors = pal, opacity = .8, group = group_name[i])
}, .init = map)
